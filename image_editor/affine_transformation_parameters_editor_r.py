...
""")
    ...
    def afftrans(self, data, px, py, rz, sx, sy, title):
        ...
        # output_dir = Path( data + '_rot' )
        output_dir = Path(src_dir.parent/f'rot_{src_dir.name}')
        ...
        out_info ={'file':data, 'out_dir': str(output_dir),'posxy':(px,py), 'rot_deg':rz, 'scalexy':(sx,sy)}
        trans_out_name = output_dir / f'trans_{now}.json'
        with open(str(trans_out_name), 'w') as f:
            json.dump(out_info, f)
        print(data)
        print(f'posxy:{px},{py}, rot_deg:{rz}, scalexy:{sx},{sy}')
        
        #
        try:
            screenshot = ImageGrab.grab()
            scr_out_name = output_dir / f'scs_{now}.png'
            screenshot.save(str(scr_out_name))
        ...
        for i,f in enumerate(files):
            ...
            # cv2.BORDER_CONSTANT: iiiiii|abcdefgh|iiiiiii (default, 0)
            # cv2.BORDER_REPLICATE: aaaaaa|abcdefgh|hhhhhhh

            # interpolationFlag : cv2.INTER_LINEAR -> bilinear interpolation 
            # https://pystyle.info/opencv-wrap-affine/
            
            tiff.imwrite(output_dir / f'rot_{f.name}', dst, compression=None)
